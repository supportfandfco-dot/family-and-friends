...
export async function sendMessage(chatId, senderId, content, type = 'text', extra = {}) {
  ...
  await supabase.from('chats').update({
    last_message: { content, type, sender_id: senderId, id: msg.id, created_at: msg.created_at },
  }).eq('id', chatId);
...
export function subscribeToHiddenChatsSettings(uid, callback) {
  supabase.from('hidden_chats_settings').select('*').eq('user_id', uid).maybeSingle()
    .then(({ data }) => callback(data || { pin_hash: null, hidden_chat_ids: [] }));
  const channel = supabase
...
export function subscribeToGroups(uid, callback) {
  ...
  const refresh = async () => {
    const { data } = await supabase
      .from('group_members').select('group_id, groups(*)').eq('user_id', uid);
    groups = (data || []).map(row => row.groups);
    callback(groups);
  };
  refresh();
  const channel = supabase
