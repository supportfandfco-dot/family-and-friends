        ...
        <div className="flex items-center gap-1.5 overflow-x-auto flex-1" style={{ scrollbarWidth: 'none' }}>
          {smartRepliesLoading && smartReplies.length === 0 ? (
            ...
            smartReplies.map((r, i) => (
              <button key={i} onClick={() => onReply?.(r.value)}
                className="flex-shrink-0 px-3 py-1.5 rounded-full text-[12px] transition-all active:scale-95 flex items-center justify-center gap-1.5"
                style={{
                ...
                {r.type === 'schedule' && <span className="text-[12px]">📅</span>}
                {r.type === 'call' && <span className="text-[12px]">📞</span>}
                {r.type === 'remind' && <span className="text-[12px]">⏰</span>}
                {r.type === 'forward' && <span className="text-[12px]">📤</span>}
                {r.value}
              </button>
            ))
          )}
