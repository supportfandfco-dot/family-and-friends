    ...
    <div className="flex flex-col h-full bg-[var(--bg-primary)]">
      ...
      <div className="flex gap-2 px-4 pt-4">
        <button onClick={() => switchMode('name')}
          className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-1.5 ${mode === 'name' ? 'bg-brand-500 text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)]'}`}>
          <Search size={14}/> By Name
        </button>
        <button onClick={() => switchMode('code')}
          className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-1.5 ${mode === 'code' ? 'bg-brand-500 text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)]'}`}>
      ...
      <div className="px-4 pt-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-3 rounded-2xl bg-[var(--bg-secondary)] text-[var(--text-primary)] placeholder-[var(--text-secondary)] border border-[var(--border)] focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
          />
          {query && (
      ...
      <div className="flex-1 overflow-y-auto px-4 mt-3 space-y-2">
        ...
        {!loading && results.map(u => (
          <div key={u.id} className="flex items-center gap-3 p-3 rounded-2xl bg-[var(--bg-secondary)]">
