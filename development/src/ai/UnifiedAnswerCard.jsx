    ...
    <div className="rounded-xl p-4 transition-all duration-300 cursor-pointer active:scale-[0.99]"
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {hasText && (
            <p className="text-sm text-[var(--text-secondary)] leading-snug line-clamp-2 mt-0.5">
              {result.text}
            </p>
          )}
          {expanded && hasText && (
            <p className="text-sm text-[var(--text-primary)] leading-relaxed mt-2 pt-2 border-t border-[var(--border)]">
              {result.text}
            </p>
          )}
        </div>
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center px-2.5 py-1 font-bold text-[10px] tracking-wider rounded-full"
            style={{
      ...
      {isMultiModel ? (
        <div className="flex flex-col gap-4">
          <UnifiedBlock
            ...
            error={!unified && !unifiedLoading && !loading ? 'Synthesis failed.' : null}
          />
          <div>
            <div className="flex items-center justify-between mb-2 px-1">
              <h3 className="text-[11px] text-[var(--text-secondary)] font-bold uppercase tracking-widest">
                Source Perspectives
              </h3>
              <span className="text-[11px] text-brand-500 font-mono">3 Insights</span>
            </div>
            <div className="flex flex-col gap-2">
              {Object.keys(MODELS).map(modelId => (
                <ModelCard key={modelId} model={modelId} result={responses[modelId]}/>
              ))}
            </div>
          </div>
        </div>
      ) : (
        <UnifiedBlock text={text} loading={loading} error={error}/>
      )}
